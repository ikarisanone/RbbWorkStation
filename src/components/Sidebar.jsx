    ...
    ease: 'easeInOut'
      ...
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center justify-between">
          ...
          delay: 0.2
              ...
              <div>
                <h1 className="text-xl font-bold text-white">{appSettings?.app_name || 'Proje Kalkanı'}</h1>
                <p className="text-xs text-gray-400">{appSettings?.app_description || 'Güvenli Proje Yönetimi'}</p>
              </div>
            </motion.div>}
      ...
      <nav className="flex-1 p-4">
        <ul className="space-y-2">
          ...
          const isActive = location.pathname === item.path || (item.path.startsWith('/projeler') && location.pathname.startsWith('/projeler'));
